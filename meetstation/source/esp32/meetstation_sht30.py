'''
    Weather Station (c)2023 Effevee
    ...
    Hardware : - DOIT ESP32 DEVKit v1 dev board
               - SHT30 temperature and humidity outdoor sensor board
    ...
    Wiring :    ESP32      SHT30     BMP180     BH1750FVI     OLED     Debug   18650 battery
                Pin GPIO    Pin        Pin         Pin        Pin       ON     voltage monitor
                --------   -----     ------     ---------     ----     -----   ---------------
                3V           2         VIN         VCC        VCC
                GND          1         GND         GND        GND
                D22  22      4         SCL         SCL        SCL
                D21  21      3         SDA         SDA        SDA
                D5   5                                                  GND
...
def get_sensor_readings():
    ...
    ################################################################################
    sht30 = SHT30(scl_pin=config.SCL_PIN, sda_pin=config.SDA_PIN, i2c_address=0x44)
    
    # check if SHT30 sensor is detected
    if not sht30.is_present():
        raise RuntimeError('Cannot find SHT30 sensor')
    
    # read SHT30 sensor
    temperature, humidity = sht30.measure()
    sht30_temp = temperature_2_unit(temperature)
    sht30_hum = humidity
    
    if debug_on():
        print('')
        print('SHT30       T: {:.2f} {} - H: {:.2f} %' .format(sht30_temp, 'F' if config.FAHRENHEIT else 'C', sht30_hum))
    ...
    return {'sht30_temp': sht30_temp, 'sht30_hum': sht30_hum, 'bmp180_temp': bmp180_temp, 'bmp180_pres': bmp180_pres, 'bmp180_alt': bmp180_alt, 'bh1750_lum': bh1750_lum, 'bat_volt': bat_volt}
...
def log_readings(ow_data, sensor_data, wifi_rssi):
    ...
    ac_temp = sensor_data['sht30_temp']
    ac_hum  = sensor_data['sht30_hum']
